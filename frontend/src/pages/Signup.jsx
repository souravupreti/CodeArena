    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0f0f0f] p-6 text-white">
      <div className="w-full max-w-md rounded-lg border border-[#303030] bg-[#242424] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.4)]">
        <div className="mb-8 flex flex-col items-center">
          ...
          <p className="mt-1 text-sm text-[#a3a3a3]">Create your competitive profile</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="mb-1.5 block font-mono text-xs font-semibold uppercase text-[#b8b8b8]">First Name</label>
            <div className="relative">
              <input
                type="text"
                placeholder="John"
                className={`w-full rounded-md border bg-[#1a1a1a] px-3.5 py-2.5 pl-10 font-mono text-sm text-white outline-none transition placeholder:text-[#747474] ${errors.firstName ? 'border-[#ff375f]' : 'border-[#3a3a3a] focus:border-[#ffa116]'}`}
                {...register('firstName')}
              />
              <User size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8a8a8a]" />
            </div>
            {errors.firstName && <span className="mt-1 block font-mono text-xs text-[#ff6b6b]">{errors.firstName.message}</span>}
          </div>
        ...
        <div className="mt-6 border-t border-[#303030] pt-6 text-center">
          <span className="font-mono text-xs text-[#a3a3a3]">
            Already have an account? <NavLink to="/login" className="font-semibold text-[#ffa116] hover:text-[#ffb84d] hover:underline">Sign In</NavLink>
          </span>
        </div>
      </div>
    </div>
